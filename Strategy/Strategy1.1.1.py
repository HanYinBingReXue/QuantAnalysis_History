...
# Time : 2022 Dec 13
# Goal : 本代码是一个买卖策略
# Process : 计算MA99 当平均价格高于MA99时则开多，当平均价格低于MA99时则开空
...
if len(raw_eth_price_data > 50000):
    raw_eth_price_data = raw_eth_price_data[-40001:-1];
print("Time: from %s to %s" %(raw_eth_price_data.iloc[0].name,raw_eth_price_data.iloc[-1].name))
...
for i in range(MA_NUM,len(price)):
    ...
    if i >= 110:
        ...
        if price[i] >= MA_price and transaction_state_value == 0 and 0: #平均价格大于MA99#开多
        ...
        if price[i] <= MA_price and transaction_state_value == 0: #平均价格小于MA99开空
            transaction_state_value = 1;
            transaction_type_value = -1;
            transaction_store.append([raw_eth_price_data.iloc[i].name,
                                    transaction_type[transaction_type_value],
