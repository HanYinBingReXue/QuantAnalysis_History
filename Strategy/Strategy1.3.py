# Author: Qi
# Time : 2022 Dec 13
# Goal : 本代码是对1.2的一个对比，使用斜率为买卖策略
# Process : 
# 使用3个bar计算bar结束时的斜率，当bar1开仓（何时买）
# 当开单时，计算前N个蜡烛的平均波动率 
# 价格回落至线平仓线时平仓
# 
# 并将最后的结果用图像进行了可视化
...
for i in range(MA_NUM,len(price)):
    ...
    up_limit_value = average_price + 4 * standard_deviation # 更新开多标准
    up_limit.append(up_limit_value)
    down_limit_value = average_price - 4 * standard_deviation # 更新开空标准
