...
def stratagy_test(df,switch,imageflag): #本function时为了测试买卖时的结果
    ...
    exp12     = price.ewm(span=7, adjust=False).mean()
    exp26     = price.ewm(span=14, adjust=False).mean()
    macd      = exp12 - exp26
    ...
    for i in range(0,len(macd_deriv)):
        if macd_deriv[i] > 0:
            macd_deriv_list.append(-1)
        else:
            macd_deriv_list.append(1)

    for i in range(0,len(price)):
            # 对三个点进行判断，bar-2 > bar-1 < bar0 
            if  macd_deriv_list[i] == 1 \
            ...
            if  macd_deriv_list[i] == -1 and transaction_state_value == 1 and transaction_type_value == 1 : #平多
                transaction_state_value = -1;
            ...
            if  macd_deriv_list[i] == 1 and transaction_state_value == 1 and transaction_type_value == -1 : #平空
                transaction_state_value = -1;
                transaction_type_value = -2;
                transaction_store.append([df.iloc[i].name,
                                        transaction_type[transaction_type_value],
