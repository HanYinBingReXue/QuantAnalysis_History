...
def side_test(df,switch,MA_NUM,bar_interval): #本function时为了测试单边买卖时的结果
...
# price = raw_eth_price_data["Open"]
    ...
    for i in range(MA_NUM,len(price)):
        if i >= 110:
            ...
            if price[i] >= MA_price and transaction_state_value == 0 and long_state: #平均价格大于MA99#开多
            ...
            if price[i] <= MA_price and transaction_state_value == 0 and short_state: #平均价格小于MA2开空
                transaction_state_value = 1;
                transaction_type_value = -1;
                transaction_store.append([df.iloc[i].name,
                                        transaction_type[transaction_type_value],
