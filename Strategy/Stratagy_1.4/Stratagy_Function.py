...
# price = raw_eth_price_data["Open"]
    ...
    volatility_list = [];
    delta_coeffi = 2.7;
    average_vlolatility_value = 0;
    market_state_value = 0;
    for i in range(0,len(price)):
        volatility_value = (df.iloc[i]["High"] - df.iloc[i]["Low"]) / df.iloc[i]["Open"];
        volatility_list.append(volatility_value);
        if i >= Small_MA:
            MA_price_small = average_cal(price[i-Small_MA+1:i+1],Small_MA); # 更新MA平均价格
            MA_list_small.append(MA_price_small);
            # average_vlolatility_value = average_cal(volatility_list[i-Small_MA+1:i+1],Small_MA);
        if i >= Midlle_MA:
            MA_price_middle = average_cal(price[i-Midlle_MA+1:i+1],Midlle_MA); # 更新MA平均价格
            MA_list_middle.append(MA_price_middle);
            standard_deviation = standard_deviation_cal(price[i-Midlle_MA+1:i+1]); # 更新标准差
            standard_deviation_list.append(standard_deviation);
        if i >= Big_MA:
            MA_price_big = average_cal(price[i-Big_MA+1:i+1],Big_MA); # 更新MA平均价格
            MA_list_big.append(MA_price_big);

            if transaction_type_value == 1: # 更新平多标准
                highest_price = max(highest_price,df.iloc[i]["High"])
                close_long_price = highest_price  - 2 * standard_deviation;
            if transaction_type_value == -1: # 更新平空标准
                lowest_price = min(lowest_price,df.iloc[i]["Low"]) 
                close_short_price =  lowest_price + 2 * standard_deviation;
            
            n, bins, patches = plt.hist(volatility_list[i-5:i+1], 10, density=True, facecolor='g', alpha=0.75);
            # print(bins[n.argmax()]) #最多可能波动率值
            # print(bins[n.argmax()] * 1.5) #这以下都算震荡
            # print(bins[n.argmax()] * 5) #这以上都算趋势

            if  bar_is_downward(df.iloc[i-1]) and bar_is_upward(df.iloc[i]) \
            and min(df.iloc[i-6:i]["Low"]) == df.iloc[i-1]["Low"] \
            and transaction_state_value == 0 and long_state == 1: #开多
            ...
            if  bar_is_upward(df.iloc[i-1]) and bar_is_downward(df.iloc[i]) \
                and max(df.iloc[i-6:i]["High"]) == df.iloc[i-1]["High"] \
                and transaction_state_value == 0 and short_state == 1: #平均价格小于MA2开空
                transaction_state_value = 1;
                transaction_type_value = -1;
                transaction_store.append([df.iloc[i].name,
                                        transaction_type[transaction_type_value],
    ...
    if imageflag == 1:
        ...
        fig.add_trace(go.Scatter(x = df.index[Small_MA:], y=MA_list_small,mode='lines',line_color = "#00000f",
                    name='MA_small'),row=1,col=1)
        fig.add_trace(go.Scatter(x = df.index[Midlle_MA:], y=MA_list_middle,mode='lines',line_color = "#f00000",
                    name='MA_middle'),row=1,col=1)
        fig.add_trace(go.Scatter(x = df.index[Big_MA:], y=MA_list_big,mode='lines',line_color = "#00ff00",
