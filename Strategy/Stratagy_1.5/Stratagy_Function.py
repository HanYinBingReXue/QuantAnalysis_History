...
# price = raw_eth_price_data["Open"]
    ...
    deviation_MA_Num = 5;
    b,a = signal.butter(6,0.07,"lowpass") #初始化滤波器
    lowpass_padlen = 22;
    for i in range(0,len(price)):
        
        if i >= deviation_MA_Num:
            standard_deviation = standard_deviation_cal(price[i-deviation_MA_Num+1:i+1]); # 更新标准差
        ...
        if i >= lowpass_padlen:
            ...
            filterdata = signal.filtfilt(b,a,price[:i+1]) #对当前值以及之前的值进行低通滤波
            # 对三个点进行判断，bar-2 > bar-1 < bar0 
            if  filterdata[i] > filterdata[i-1] and filterdata[i-3] > filterdata[i-1] \
            ...
            # 对三个点进行判断，bar-2 < bar-1 > bar0 多反转 开空    
            if  filterdata[i] < filterdata[i-1] and filterdata[i-3] < filterdata[i-1] \
            ...
            if (filterdata[i] < filterdata[i-1] and filterdata[i-2] < filterdata[i-1]) and transaction_state_value == 1 and transaction_type_value == 1 : #平多
                transaction_state_value = -1;
            ...
            if (filterdata[i] > filterdata[i-1] and filterdata[i-2] > filterdata[i-1]) and transaction_state_value == 1 and transaction_type_value == -1 : #平空
                transaction_state_value = -1;
                transaction_type_value = -2;
                transaction_store.append([df.iloc[i].name,
                                        transaction_type[transaction_type_value],
