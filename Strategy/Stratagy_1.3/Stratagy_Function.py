...
# price = raw_eth_price_data["Open"]
    ...
    for i in range(0,len(price)):
        if i >= Big_MA:
            ...
            # 偏离足够大，且不能刚偏离就买
            if MA_price_small < MA_price_middle and MA_price_middle < MA_price_big \
                and (MA_price_middle - MA_price_small) > delta_coeffi * (MA_price_big - MA_price_middle) \
                and transaction_state_value == 0 and long_state == 1: #平均价格大于MA99#开多
            ...
            if MA_price_small > MA_price_middle and MA_price_middle > MA_price_big and \
                 (MA_price_small - MA_price_middle) > delta_coeffi * (MA_price_middle - MA_price_big) and \
                transaction_state_value == 0 and short_state == 1: #平均价格小于MA2开空
                transaction_state_value = 1;
                transaction_type_value = -1;
                transaction_store.append([df.iloc[i].name,
                                        transaction_type[transaction_type_value],
    ...
    if imageflag == 1:
        ...
        fig.add_trace(go.Scatter(x = df.index[Big_MA:], y=MA_list_small,mode='lines',line_color = "#00000f",
                    name='MA_small'),row=1,col=1)
        fig.add_trace(go.Scatter(x = df.index[Big_MA:], y=MA_list_middle,mode='lines',line_color = "#f00000",
